...
const AppointmentModal = ({ show, date, onClose, timeSlots, onDateChange, selectedSlots, setSelectedSlots }) => {
  ...
  useEffect(() => {
    // TODO
    // get dentists, 
    // store dentist and services in session storage.
  ...
  const handlePayment = () => {
    // TODO: pass state and redirect to payment page.
    // navigate('/payment', { state: { selectedSlots, date, selectedService, selectedDoctor   } });
    ...
    <div className="fixed inset-0 bg-black bg-opacity-30 z-50 flex justify-center items-center">
      <div className="bg-white p-6 rounded shadow-lg w-96 ">
        ...
        <div className="flex justify-between">
          ...
          <button disabled className="px-4 py-2 bg-gray-400 text-white rounded hover:bg-gray-600 disabled:bg-gray-500" onClick={handlePayment}>Make Payment</button>
        </div>
      </div>
    </div>
  );
